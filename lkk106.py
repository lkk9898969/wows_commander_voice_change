import Windows,winreg,icon,traceback
...
class MainWindow_controller(QtWidgets.QStackedWidget):
    ...
    def mod_change(self):
        self.backup()
        try:
            tree=et.parse(paths+xmlfile.format(""))
            root=tree.getroot()
            for child in root.iter("voice"):
                try:
                    for voice in child:
                        if voice.tag=="mod":
                            voice.text=self.mod
                    ...
                    pass
            tree.write(paths+xmlfile.format(""))
            return False
        except:
            traceback.print_exc(file=open("lkk106.log",'w'))
            return True
